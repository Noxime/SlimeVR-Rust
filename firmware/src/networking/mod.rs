...
pub async fn protocol_task(packets: &'static Packets) -> ! {
	...
	join(periodic(packets), responsive(packets)).await.0
}

// Handles parts of the protocol where we send packets based on a timer
async fn periodic(packets: &Packets) -> ! {
	...
	loop {
		...
		if *packets.connection.lock().await == Connection::Offline {
			packets
				.serverbound
...
// Handles the part of the protocol where we send responses to queries
async fn responsive(packets: &Packets) -> ! {
	loop {
		match packets.clientbound.recv().await {
			// Identify ourself when discovery packet is received
			CbPacket::Discovery => {
				*packets.connection.lock().await = Connection::Offline;
			}
			CbPacket::Heartbeat => {
